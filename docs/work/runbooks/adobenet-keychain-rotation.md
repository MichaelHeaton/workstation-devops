# Runbook: Rotate ADOBENET in Keychain Access (Vault `vl` / Okta)

Adobe requires periodic **ADOBENET** password changes (typically **every 90 days**). The copy stored for **`vl` / `vault_mgmt`** lives in **Keychain Access**, not in the Passwords app — update it whenever you change ADOBENET, or `security` will keep feeding Vault the old password.

**Ansible reference:** `vault_okta_keychain_service` and `adobe_username` in `group_vars/work.yml` (defaults: service `adobe-vault-okta`, account matches LDAP short name).
...
- After changing ADOBENET anywhere else (Okta, laptop login policy, etc.) if your Vault CLI login suddenly fails with bad password while MFA still works conceptually.
...
- Have the **new** ADOBENET password available (post-rotation).
...
4. Use the search field: type **`adobe-vault-okta`** (or your `vault_okta_keychain_service` if you overrode it).
...
7. Replace the value with the **new** ADOBENET password.
...
|---------|----------------|-----|
| `security: … could not be found` | Wrong service/account or item only in Passwords app | Recreate item in Keychain Access; match `group_vars/work.yml`. |
